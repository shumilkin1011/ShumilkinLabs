...
namespace MultiParse
{
    ...
    public class Expression
    {
        ...
        private object Compile()
        {
            ...
            while (!String.IsNullOrEmpty(compileExpression))
        ...
        private void ProcessToken(ref string expression)
        {
                ...
                if (len > 0)
                {
                    ...
                    while (true)
                    {
                            ...
                            || (op.Precedence < op2.Precedence))
                        {
                            ...
                            // Compile
                            CompiledAction action = delegate() { op2.Execute(outputStack); };
                            action();
            ...
            foreach (MPDataType dt in typeList)
            {
                ...
                {
                    expression = expression.Substring(len);
                    CompiledAction action = delegate() { outputStack.Push(converted); };
                    action();
                    compiledQueue.AddLast(action);
                    lastToken = converted;
                    return;
            ...
            if (len > 0)
            {
                ...
                else
                {
                    ...
                    if ((operatorStack.Count > 0) && (operatorStack.Peek() is MPFunction))
                    {
                        ...
                        CompiledAction action = delegate() { fun.Execute(outputStack, arguments); };
                        action();
                        compiledQueue.AddLast(action);
                        argumentCounts.RemoveLast();
                    }
        ...
        private void CompileToBracket(bool popBracket)
        {
            while (true)
            {
                ...
                if (pop is MPOperator)
                {
                    ...
                    CompiledAction action = delegate() { op.Execute(outputStack); };
                    action();
                    compiledQueue.AddLast(action);
                }
            }
